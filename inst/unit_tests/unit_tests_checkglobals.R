...
check8.14 <- as_sarif_json(check1, pretty = TRUE)
check8.15 <- as_sarif_json(check4.1, use_cli = FALSE, pretty = TRUE)
...
dotest("8.14", length(gregexpr("DESCRIPTION", check8.12)[[1]]), 5L)
dotest("8.15", file.info(check8.13)$size > 0, TRUE)
dotest("8.16", inherits(check8.14, "json"), TRUE)
dotest("8.17", length(gregexpr('\\"functions1\\.R\\"', check8.14)[[1]]), 26L)
dotest("8.18", inherits(check8.15, "json"), TRUE)
dotest("8.19", length(gregexpr('\\"functions1\\.R\\"', check8.15)[[1]]), 26L)
